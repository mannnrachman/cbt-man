  ...
  .handler(async ({ data }) => {
    try {
      ...
      if (caller.role === "admin_prodi" || caller.role === "evaluator") {
        ...
        if (item.pesertaId !== caller.id) return { ok: false as const, error: "Forbidden" };
        ...
        if (!existing) {
          const ujianData = await prisma.ujian.findUnique({
            where: { id: item.ujianId },
...
export const actionLiveSesiServer = createServerFn({ method: "POST" })
  .validator(
    z.object({ sesiId: z.string().min(1), action: z.enum(["forceSubmit", "resetPelanggaran"]) }),
  )
  .handler(async ({ data }) => {
    try {
      const caller = await requireCaller();
      ...
      if (
        !sesi ||
        (caller.role !== "super_admin" && !(await operatorCanTouchUjian(caller, sesi.ujianId)))
      ) {
        return { ok: false as const, error: "Forbidden" };
      }
      if (data.action === "forceSubmit") {
        // Grade server-side so a forced submit also stores the same
        // authoritative score as a normal submit.
...
export const getLiveOnlineSesis = createServerFn({ method: "GET" }).handler(async () => {
  const caller = await requireCaller();
  ...
  for (const r of rows) {
    if (caller.role === "super_admin" || (await operatorCanTouchUjian(caller, r.ujianId)))
      scoped.push(r);
  }
  return scoped.map((r: any) => {
    const soalIds = parseJson<string[]>(r.soalIds, []);
    const jawaban = parseJson<any[]>(r.jawaban, []);
    const dijawab = jawaban.filter(
      (j: any) =>
        (j.jawabanIds && j.jawabanIds.length > 0) || (j.jawabanEssay && j.jawabanEssay.length > 0),
    ).length;
    ...
    return {
      ...
      user: r.peserta
        ? { namaLengkap: r.peserta.namaLengkap, username: r.peserta.username }
        : undefined,
...
export const createSesiServer = createServerFn({ method: "POST" })
  .validator(z.object({ ujianId: z.string() }))
  .handler(async ({ data }) => {
    try {
      await seedIfNeeded();
      const caller = await requireCaller();
      ...
      for (const ts of ujian.topicSets) {
        ...
        if (chosen.length < ts.jumlah) {
          return { ok: false as const, error: `Bank soal untuk topik terkait ujian "${ujian.nama}" belum cukup.` };
        }
        soalTerpilih.push(...chosen);
      }

      if (soalTerpilih.length === 0) {
        return { ok: false as const, error: `Bank soal untuk ujian "${ujian.nama}" belum cukup atau tidak cocok dengan filter topik.` };
      }
      ...
      await prisma.sesiUjian.create({
        data: {
          ...
          jawaban: stringifyJson(soalTerpilih.map((s) => ({
            soalId: s.id,
            jawabanIds: [],
            jawabanEssay: "",
            ragu: false,
