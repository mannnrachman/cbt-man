  ...
  return (
    <div className="mt-3 flex flex-col gap-3">
      ...
      {activeTab === "nilai" && ujian.allowNilaiNormal && (
        <div className="p-2 sm:p-3 bg-emerald-50/40 border-2 border-emerald-200/60 rounded-2xl dark:bg-emerald-950/20 dark:border-emerald-900/50 max-h-80 overflow-x-auto overflow-y-auto animate-in zoom-in-95 fade-in duration-200 [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-emerald-500/50 [&::-webkit-scrollbar-thumb]:rounded-full">
          <NilaiNormalTable />
        </div>
      )}
    </div>
...
function RouteComponent() {
  ...
  useEffect(() => {
    ...
    const active = sesiRepo
      .all()
      .find((x) => x.ujianId === ujian.id && x.pesertaId === user.id && x.status === "sedang");
    setSesi(active || null);
    setSesiDicari(true);
  }, [user, ujian]);
      ...
      try {
        ...
        if (latestSesi) {
          if (latestSesi.status === "selesai") {
            ...
            latestSesi.endsAt &&
            sesiRef.current &&
            latestSesi.endsAt !== sesiRef.current.endsAt
          ) {
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="hidden sm:block">
                ...
                <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 leading-none">
                  Dari {currentSesi.soalIds.length} Soal
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 sm:gap-6">
              ...
              <div className="hidden sm:flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700/50">
                ...
                <button
                  ...
                  className={cn(
                    "flex items-center justify-center w-8 h-8 rounded-lg font-bold text-base transition-all",
                    fontSize === "base"
                      ? "bg-white dark:bg-slate-700 text-primary shadow-sm"
                      : "text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200",
                  )}
                ...
                <button
                  ...
                  className={cn(
                    "flex items-center justify-center w-8 h-8 rounded-lg font-bold text-lg transition-all",
                    fontSize === "lg"
                      ? "bg-white dark:bg-slate-700 text-primary shadow-sm"
                      : "text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200",
                  )}
              ...
              >
                <Clock
                  className={cn(
                    ...
                    critical ? "text-white" : danger ? "text-red-500" : "text-slate-400",
                  )}
                />
              ...
              <Button
                variant="outline"
                size="icon"
                className="md:hidden w-10 h-10 rounded-xl border-slate-200 dark:border-slate-700"
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden relative">
            <div className="max-w-5xl mx-auto px-5 sm:px-10 py-6 sm:py-10 pb-32">
              ...
              <div
                aria-live="polite"
                className={cn(
                  "prose prose-slate dark:prose-invert max-w-none mb-10 text-slate-800 dark:text-slate-200 leading-loose",
                  textSizeClass,
                )}
              >
              ...
              {idx < currentSesi.soalIds.length - 1 ? (
                ...
                <Button
                  ...
                  onClick={() => {
                    if (
                      confirm(
                        "Pastikan semua jawaban telah terisi dengan benar. Yakin kumpulkan ujian sekarang?",
                      )
                    )
                      void submit();
                  }}
        ...
        <div className="hidden md:flex flex-col w-[350px] lg:w-[450px] bg-slate-50/50 dark:bg-slate-950/30 border-l border-slate-200 dark:border-slate-800">
          <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm">
            ...
            <div className="mt-4 flex flex-col gap-2">
              ...
              <div className="flex items-center gap-3 text-sm font-medium text-slate-600 dark:text-slate-400">
                <div className="w-4 h-4 rounded-full bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 shadow-sm" />{" "}
                Belum Dijawab
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            <div className="grid grid-cols-6 lg:grid-cols-8 gap-2">
              {currentSesi.soalIds.map((_, i) => {
                ...
                const dijawab =
                  (a?.jawabanIds?.length ?? 0) > 0 ||
                  (a?.jawabanEssay ? a.jawabanEssay.length > 0 : false);
          ...
          <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm">
            <Button
              ...
              onClick={() => {
                if (confirm("Yakin ingin mengumpulkan?")) void submit();
              }}
            >
              Akhiri Ujian
            </Button>
