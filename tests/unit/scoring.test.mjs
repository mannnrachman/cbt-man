/**
 * Unit tests for the canonical scoring rules in `src/lib/cbt/scoring.ts`.
 *
 * These rules are shared by the participant submit path (client), the
 * server-side authoritative grading in `src/lib/server/sesi/functions.ts`,
 * and the evaluasi/recompute flow, so any change here affects all of them.
 *
...
test("gradeAnswers gives poinSalah for a wrong answer and poinKosong for empty", () => {
  ...
  const { jawaban: graded, skorTotal } = gradeAnswers(u, soalById, [
    jawaban(PG.id, ["b"]), // salah -> -1
    jawaban(BS.id, []), // kosong -> 0.5
  ]);
...
test("gradeAnswers computes maxSkor as soal count times poinBenar including essays", () => {
  ...
  const { skorTotal, maxSkor } = gradeAnswers(u, soalById, [
    jawaban(PG.id, ["a"]), // benar -> 2
    jawaban(BS.id, ["a"]), // salah -> 0
    jawaban(MULTI.id, []), // kosong -> 0
    jawaban(ESSAY.id, [], "esai"), // belum dinilai -> 0
  ]);
  assert.equal(skorTotal, 2);
